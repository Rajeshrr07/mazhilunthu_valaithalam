...
export async function processCarImageWithAI(file: File): Promise<{
  success: boolean;
  data?: any;
  error?: string;
}> {
...
export async function updateCarStatus(
  id: string,
  { status, featured }: UpdateCarStatusData
): Promise<{ success: boolean; error?: string }> {
  try {
    const { userId } = await auth();
    if (!userId) throw new Error("Unauthorized");

    const updateData: UpdateCarStatusData = {};
